...
TEST(NOP, simpleTestWithFunction) {
  auto desiredFunction = [](std::vector<float> x, std::vector<float> q) {
    return (pow(x[0], 2) - pow(x[1], 2)) * cosf(q[0] * x[0] + q[1]) +
           x[0] * x[1] * exp(-q[2] * x[0]);
  };
...
TEST(NOPminPsi, multPsi) {
  ...
  for (int i = 1; i <= 28; i++) {
    ...
    netOper.setPsi({{0, 0, i, 0}, {0, 0, 0, i}, {0, 0, 1, 0}, {0, 0, 0, 1}});
...
TEST(NOPminPsi, binarPsi) {
  ...
  for (int i = 1; i <= 8; i++) {
    netOper.setPsi({{0, 1, 0}, {0, i, 1}, {0, 0, 1}});
...
TEST(NOP, trainedOperatorTest) {
  ...
  std::vector<SliceT> slice_pack = {
      {2.5000000000000000E+000, 2.5000000000000000E+000,
       1.3100000000000001E+000, 7.6197997817352853E+003,
       -9.4771230671817757E+003},
      {2.5000000000000000E+000, 2.5000000000000000E+000,
       -1.3100000000000001E+000, 1.4423954467471624E+001,
       -3.9757176807666457E+000},
      {2.5000000000000000E+000, -2.5000000000000000E+000,
       -1.3100000000000001E+000, -5.5207311836960698E+003,
       5.8007645332198299E+003},
      {4.6561580083458731E-003, 4.0351029521603278E-002,
       8.3948216438293491E-002, 1.3335461264421879E+001,
       8.4973200088496483E+000},
      {-9.1758697199960415E-001, -4.4754665005156380E-001,
       -5.1089064121246375E-001, -3.4732514696068890E+001,
       -6.2624360542277708E+000},
      {5.6599015741103036E-002, 4.1273824183417351E-002,
       -2.5226671345531987E-001, -2.0985952049623924E-001,
       2.6154650032382927E+000},
  };
  std::vector<SliceT> y_out_pack;
  std::vector<SliceT> y_out_gold_pack;
